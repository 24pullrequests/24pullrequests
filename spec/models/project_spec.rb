...
describe Project do
  ...
  it { should validate_uniqueness_of(:github_url).with_message(
    'Project has already been suggested.') }
  ...
  it "Validation should not pass on wrong programming language" do
    ...
    project.errors[:main_language].include?('must be a programming language'
      ).should be_true
  end
