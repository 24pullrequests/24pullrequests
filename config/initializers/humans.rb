  ...
  class Application < Rails::Application
    config.after_initialize do
      if Rails.env.production?
        ...
        Rails.application.config.octokit_client = Octokit::Client.new(access_token: access_token,
                                                    auto_paginate: true)

        config.contributors = begin
          Timeout::timeout(5) {
            Rails.application.config.octokit_client.contributors('24pullrequests/24pullrequests')
          }
        rescue => e
