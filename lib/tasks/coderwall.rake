    ...
    if api_key.present?
      User.all.each do |user|
        if user.pull_requests.year(CURRENT_YEAR).any?
          payload = {github:user.coderwall_username, badge:"TwentyFourPullRequestsParticipant", date:"12/25/#{CURRENT_YEAR}", api_key:api_key}
        ...
        if user.pull_requests.year(CURRENT_YEAR).length > 23
          payload = {github:user.coderwall_username, badge:"TwentyFourPullRequestsContinuous", date:"12/25/#{CURRENT_YEAR}", api_key:api_key}
          resp = conn.post '/award', payload.to_json, 'Content-Type' => 'application/json', :accept => 'application/json'
        end
      end
    end
